...
SPECIALIST_TOOLSETS = {
    ...
    "studio-grok": "web,x_search,image_gen,vision,file,terminal",
}


@dataclass(frozen=True)
class _SpecialistDispatch:
    profile: str
    task: str
    timeout: int
    clip_id: str | None
...
def _handoff_event(
        store: JobStore | None, job_id: str, profile: str,
        event_type: JobEventType, summary: str, *,
        phase: JobPhase, detail: dict | None = None) -> None:
    if store is not None:
        store.append_job_event(
            job_id, profile, event_type, summary,
            phase=phase, detail=detail)


def _terminate_handoff(process: subprocess.Popen) -> None:
...
def _dispatch_specialist(
        root: Path, project: str, request: _SpecialistDispatch) -> str:
    profile = request.profile
    task = request.task
    timeout = request.timeout
    pp = project_path(root, project)
    scoped_clip_id, scoped_clip = _dispatch_clip_scope(
        root, pp, request.clip_id)
    ...
    bridge: HermesSessionEventBridge | None = None
    specialist_job = None
    ...
    if job_id:
        try:
            ...
            if (parent_job.status is not JobStatus.RUNNING
                    or parent_job.project != pp.name
                    or parent_job.clip_id != scoped_clip_id):
                raise ValueError(
                    "specialist handoff does not match its running parent job")
            specialist_job = replace(parent_job, profile=profile)
        except Exception as exc:
            raise RuntimeError(
                "could not attach specialist handoff observability") from exc

    with lock_path.open("a+b") as lock:
        fcntl.flock(lock.fileno(), fcntl.LOCK_EX)
        if store is not None and specialist_job is not None:
            _handoff_event(
                store, job_id, profile, JobEventType.HANDOFF_STARTED,
            ...
            if not bridge.prepare():
                _handoff_event(
                    store, job_id, profile, JobEventType.HANDOFF_FAILED,
                    "Handoff observability setup failed",
                    phase=JobPhase.FAILED)
                raise RuntimeError(
                    "could not prepare specialist handoff observability")
        try:
            ...
            _handoff_event(
                store, job_id, profile, JobEventType.HANDOFF_FAILED,
                f"{profile} failed to start", phase=JobPhase.FAILED,
    ...
    if not reply:
        _handoff_event(
            store, job_id, profile, JobEventType.HANDOFF_FAILED,
            f"{profile} returned an empty reply", phase=JobPhase.FAILED)
    ...
    _handoff_event(
        store, job_id, profile, JobEventType.HANDOFF_COMPLETED,
...
def dispatch_profile(root: Path, project: str, profile: str, task: str,
    ...
    return _dispatch_specialist(
        root, project, _SpecialistDispatch(profile, task, timeout, clip_id))
...
def dispatch_grok(root: Path, project: str, task: str,
    ...
    return _dispatch_specialist(
        root,
        project,
        _SpecialistDispatch("studio-grok", task, timeout, clip_id),
    )
