...
from studio_core.identifiers import CLIP_ID_RE, validate_clip_id
...
# Shared filesystem project-domain helpers.
...
def studio_root(override: str | None = None, *, create: bool = True) -> Path:
    ...
    if create:
        for subdirectory in ("projects", "shared", "tmp"):
...
def slugify(name: str) -> str:
    value = re.sub(
        r"[^a-z0-9-_]+", "-", name.strip().lower()).strip("-")
    if not value:
...
def create_project(root: Path, name: str, brief: str = "") -> Path:
    project = project_path(root, name, must_exist=False)
    if os.path.lexists(project):
        raise FileExistsError(f"project already exists: {project}")
    project.mkdir(parents=True)
    try:
        for subdirectory in ("references", "final", "research"):
            (project / subdirectory).mkdir()
        (project / PROJECT_BRIEF).write_text(
            f"# {name}\n\n{brief}\n\n"
            f"Created {_dt.datetime.now().isoformat(timespec='seconds')}\n",
            encoding="utf-8",
        )
        (project / "chat.jsonl").touch()
        CLIP_STORE.initialize(project, name)
    except Exception:
...
def write_prompt(root: Path, project: str, clip_id: str, prompt: str) -> Path:
    ...
    temporary = clip / f".{uuid.uuid4().hex}.current-prompt"
    try:
        temporary.write_text(prompt.rstrip() + "\n", encoding="utf-8")
